...
class model_IntEL(object):
    def __init__(self, data, para):
        ...
        self.intent_embeddings = tf.reshape(self.intent_embeddings, [-1, self.max_len, self.dim])  #[-1, max_len, dim]
        ...
        # 5 start ---------------------
        intent_input = self.intent_embeddings  # [-1, max_len, 64]
        ...
        with tf.name_scope("sab1"):
            ...
            weight = layer_dense(pxtr_input, 3)
            logits = tf.reduce_sum(weight * pxtr_dense_input, axis=2)       # logits = w_1*pxtr_1 + w_2*pxtr_2 + ...
            logits += 0.01 * tf.reduce_sum(pxtr_input, axis=2)              # aux logits
