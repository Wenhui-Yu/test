...
def print_pxtr_ndcg(epoch, train_data_input, pred_list, train_test):
    ...
    print (train_test, "ep", epoch + 1,
        "%.4f"%(sum(click_label_ndcg)/len(click_label_ndcg)), "%.4f"%(sum(lvtr_label_ndcg)/len(lvtr_label_ndcg)),
        "%.4f"%(sum(ltr_label_ndcg)/len(ltr_label_ndcg)), "%.4f"%(sum(wtr_label_ndcg)/len(wtr_label_ndcg)),
        "%.4f"%(sum(cmtr_label_ndcg)/len(cmtr_label_ndcg)), "%.4f"%(sum(ftr_label_ndcg)/len(ftr_label_ndcg)), end='   ')
    print ("%.4f"%(sum(list_lvtr_ndcg_epoch)/len(list_lvtr_ndcg_epoch)),
        "%.4f"%(sum(list_ltr_ndcg_epoch)/len(list_ltr_ndcg_epoch)), "%.4f"%(sum(list_wtr_ndcg_epoch)/len(list_wtr_ndcg_epoch)),
        "%.4f"%(sum(list_cmtr_ndcg_epoch)/len(list_cmtr_ndcg_epoch)), "%.4f"%(sum(list_ftr_ndcg_epoch)/len(list_ftr_ndcg_epoch)), end='   ')

def print_pxtr_auc(epoch, train_data_input, pred_list, train_test):
    # auc
    k = 100
    list_ltr_auc_epoch, list_wtr_auc_epoch, list_cmtr_auc_epoch, list_ftr_auc_epoch, list_lvtr_auc_epoch = [], [], [], [], []
    ltr_label_auc, wtr_label_auc, cmtr_label_auc, ftr_label_auc, lvtr_label_auc = [], [], [], [], []
    click_label_auc = []
    for i in range(len(pred_list)):  #len(pred_list)):
        # pred_list[i]     [max_len]
        # train_data_input[i]->[max_len, 13+5]      train_data_input[i][:,13] # [max_len]
        list_ltr_auc_epoch.append(auc_for_one_samp(train_data_input[i][:k,13], pred_list[i][:k], k)) # bin
        list_wtr_auc_epoch.append(auc_for_one_samp(train_data_input[i][:k,14], pred_list[i][:k], k))
        list_cmtr_auc_epoch.append(auc_for_one_samp(train_data_input[i][:k,15], pred_list[i][:k], k))
        list_ftr_auc_epoch.append(auc_for_one_samp(train_data_input[i][:k,16], pred_list[i][:k], k))
        list_lvtr_auc_epoch.append(auc_for_one_samp(train_data_input[i][:k,17], pred_list[i][:k], k))
        ...
        cmtr_label_auc.append(auc_for_one_samp(train_data_input[i][:k,5], pred_list[i][:k], k))
        ftr_label_auc.append(auc_for_one_samp(train_data_input[i][:k,6], pred_list[i][:k], k))
        lvtr_label_auc.append(auc_for_one_samp(train_data_input[i][:k,7], pred_list[i][:k], k))
    print ("%.4f"%(sum(click_label_auc)/len(click_label_auc)), "%.4f"%(sum(lvtr_label_auc)/len(lvtr_label_auc)),
        "%.4f"%(sum(ltr_label_auc)/len(ltr_label_auc)), "%.4f"%(sum(wtr_label_auc)/len(wtr_label_auc)),
        "%.4f"%(sum(cmtr_label_auc)/len(cmtr_label_auc)), "%.4f"%(sum(ftr_label_auc)/len(ftr_label_auc)), end='   ')
    print ("%.4f"%(sum(list_lvtr_auc_epoch)/len(list_lvtr_auc_epoch)),
        "%.4f"%(sum(list_ltr_auc_epoch)/len(list_ltr_auc_epoch)), "%.4f"%(sum(list_wtr_auc_epoch)/len(list_wtr_auc_epoch)),
        "%.4f"%(sum(list_cmtr_auc_epoch)/len(list_cmtr_auc_epoch)), "%.4f"%(sum(list_ftr_auc_epoch)/len(list_ftr_auc_epoch)), end='   ')

def print_pxtr_map(epoch, train_data_input, pred_list, train_test):
    # map
    k = 100
    list_ltr_map_epoch, list_wtr_map_epoch, list_cmtr_map_epoch, list_ftr_map_epoch, list_lvtr_map_epoch = [], [], [], [], []
    ltr_label_map, wtr_label_map, cmtr_label_map, ftr_label_map, lvtr_label_map = [], [], [], [], []
    click_label_map = []
    for i in range(len(pred_list)):  #len(pred_list)):
        # pred_list[i]     [max_len]
        # train_data_input[i]->[max_len, 13+5]      train_data_input[i][:,13] # [max_len]
        # list_ltr_map_epoch.append(map_for_one_samp(train_data_input[i][:k,13], pred_list[i][:k], k)) # bin
        # list_wtr_map_epoch.append(map_for_one_samp(train_data_input[i][:k,14], pred_list[i][:k], k))
        # list_cmtr_map_epoch.append(map_for_one_samp(train_data_input[i][:k,15], pred_list[i][:k], k))
        # list_ftr_map_epoch.append(map_for_one_samp(train_data_input[i][:k,16], pred_list[i][:k], k))
        # list_lvtr_map_epoch.append(map_for_one_samp(train_data_input[i][:k,17], pred_list[i][:k], k))
        ...
        cmtr_label_map.append(map_for_one_samp(train_data_input[i][:k,5], pred_list[i][:k], k))
        ftr_label_map.append(map_for_one_samp(train_data_input[i][:k,6], pred_list[i][:k], k))
        lvtr_label_map.append(map_for_one_samp(train_data_input[i][:k,7], pred_list[i][:k], k))
    print ("%.4f"%(sum(click_label_map)/len(click_label_map)), "%.4f"%(sum(lvtr_label_map)/len(lvtr_label_map)),
        "%.4f"%(sum(ltr_label_map)/len(ltr_label_map)), "%.4f"%(sum(wtr_label_map)/len(wtr_label_map)),
        "%.4f"%(sum(cmtr_label_map)/len(cmtr_label_map)), "%.4f"%(sum(ftr_label_map)/len(ftr_label_map)))
    # print ("%.4f"%(sum(list_lvtr_map_epoch)/len(list_lvtr_map_epoch)),
    #     "%.4f"%(sum(list_ltr_map_epoch)/len(list_ltr_map_epoch)), "%.4f"%(sum(list_wtr_map_epoch)/len(list_wtr_map_epoch)),
    #     "%.4f"%(sum(list_cmtr_map_epoch)/len(list_cmtr_map_epoch)), "%.4f"%(sum(list_ftr_map_epoch)/len(list_ftr_map_epoch)))
...
def layer_dense(pxtr_input, num):
    ...
    return 0.0015 * tf.stop_gradient(weight)
