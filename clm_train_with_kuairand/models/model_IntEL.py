...
class model_IntEL(object):
    def __init__(self, data, para):
        ## model hyper-params
        self.model_name = 'IntEL'
        ...
        # 2 reshape
        self.intent_re = tf.reshape(self.intent, [-1, self.max_len])
        ...
        self.intent_embeddings_table = tf.Variable(tf.random_normal([self.num, self.dim], mean=0.01, stddev=0.02, dtype=tf.float32), name='intent_embeddings_table')
        ...
        # 4 lookup
        self.intent_re = tf.reshape(self.intent_re, [-1])  # [-1, max_len] -> [bs*max_len]
        self.intent_embeddings = tf.nn.embedding_lookup(self.intent_embeddings_table, self.intent_re)  # [bs*max_len, dim]
        self.intent_embeddings = tf.reshape(self.intent_embeddings, [-1, self.max_len, self.dim])  #[-1, max_len, intent_dim]
        ...
        # 5 start ---------------------
        intent_input = self.intent_embeddings  # [-1, max_len, 48]
        ...
        pxtr_input = tf.concat([intent_input, pxtr_input], -1)
        ...
        with tf.name_scope("sab1"):
            ...
            weight = layer_dense(pxtr_input, 5)
            logits = tf.reduce_sum(weight * pxtr_dense_input, axis=2)       # logits = w_1*pxtr_1 + w_2*pxtr_2 + ...
            logits += tf.reduce_sum(pxtr_input, axis=2)                     # aux logits
